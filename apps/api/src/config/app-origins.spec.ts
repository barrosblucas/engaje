...
describe('app-origins config', () => {
  it('prioriza APP_URLS quando informado', () => {
    const origins = getAllowedAppOrigins({
      ...
      APP_URLS: 'http://localhost:3000, http://192.168.1.21:3000/, http://localhost:3000',
      NODE_ENV: 'production',
    });

    expect(origins).toEqual(['http://localhost:3000', 'http://192.168.1.21:3000']);
  });

  it('usa APP_URL como fallback quando APP_URLS não existe', () => {
    const origins = getAllowedAppOrigins({
      APP_URL: 'http://my-web.test:3000/',
      APP_URLS: undefined,
      NODE_ENV: 'production',
    });

    expect(origins).toEqual(['http://my-web.test:3000']);
    expect(
      getPrimaryAppOrigin({ APP_URL: 'http://my-web.test:3000/', NODE_ENV: 'production' }),
    ).toBe('http://my-web.test:3000');
  });

  it('permite origin de rede local em desenvolvimento', () => {
    const isAllowed = isAppOriginAllowed('http://192.168.1.21:3000', {
      APP_URL: 'http://localhost:3000',
      APP_URLS: undefined,
